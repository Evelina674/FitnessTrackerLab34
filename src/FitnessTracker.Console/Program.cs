...
while (true)
{
    ...
    {
        Console.WriteLine("Unknown command. Please enter 1, 2 or 3.");
    }
}
...
static void AddWorkoutSession(UserProfile user, FitnessTrackerService service)
{
    try
    {
        ...
        var date = DateTime.TryParse(inputDate, out var parsedDate) ? parsedDate : DateTime.Today;
        ...
        while (true)
        {
            ...
            var duration = double.TryParse(Console.ReadLine(), out var parsedDuration) ? parsedDuration : 0;
            ...
            if (type.Equals("cardio", StringComparison.OrdinalIgnoreCase))
            {
                ...
                activity = activity with { DistanceKm = double.TryParse(Console.ReadLine(), out var parsedDistance) ? parsedDistance : 0 };
            }
            else if (type.Equals("strength", StringComparison.OrdinalIgnoreCase))
            {
                Console.Write("Sets: ");
                activity = activity with { Sets = int.TryParse(Console.ReadLine(), out var parsedSets) ? parsedSets : 0 };
                Console.Write("Reps per set: ");
                activity = activity with { RepsPerSet = int.TryParse(Console.ReadLine(), out var parsedReps) ? parsedReps : 0 };
                Console.Write("Weight in kg: ");
                activity = activity with { WeightKg = double.TryParse(Console.ReadLine(), out var parsedWeight) ? parsedWeight : 0 };
            }
            else
            {
            ...
            activity = activity with { Notes = Console.ReadLine()?.Trim() ?? string.Empty };
        ...
        Console.WriteLine($"Session saved: {session.Id}");
